...
export default function StructuredJobDescriptionDialog(props: {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  staffName: string;
  jd: any | null;
}) {
  const { open, onOpenChange, staffName, jd } = props;
  if (!jd) return null;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-white max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-[#555555]">View job description for {staffName}</DialogDescription>
        </DialogHeader>

        <div className="space-y-6">
          <Card className="bg-[#f9f9f9] border border-[#e0e0e0] p-4">
            <p className="text-[#222222]" style={{ fontSize: '14px', fontWeight: 700 }}>Academic Year</p>
            <p className="text-[#555555]" style={{ fontSize: '13px' }}>{jd.academicYear || '—'}</p>
          </Card>

          <Card className="bg-[#f9f9f9] border border-[#e0e0e0] p-4">
            <p className="text-[#222222]" style={{ fontSize: '14px', fontWeight: 700 }}>Semester</p>
            <p className="text-[#555555]" style={{ fontSize: '13px' }}>{jd.semester || '—'}</p>
          </Card>

          <Card className="bg-[#f9f9f9] border border-[#e0e0e0] p-4">
            <p className="text-[#222222]" style={{ fontSize: '14px', fontWeight: 700 }}>Semester Start Date</p>
            <p className="text-[#555555]" style={{ fontSize: '13px' }}>{jd.semesterStartDate || '—'}</p>
          </Card>

          <Card className="bg-[#f9f9f9] border border-[#e0e0e0] p-4">
            <p className="text-[#222222]" style={{ fontSize: '14px', fontWeight: 700 }}>Semester End Date</p>
            <p className="text-[#555555]" style={{ fontSize: '13px' }}>{jd.semesterEndDate || '—'}</p>
          </Card>
              ...
              <table className="w-full text-sm border-collapse">
                ...
                <tbody>
                  ...
                  {(!jd.dimAcademicTasks || jd.dimAcademicTasks.length === 0) && (
                    <tr><td className="px-3 py-3 text-[#999999]" colSpan={4}>—</td></tr>
                  )}
                </tbody>
              </table>
            </div>
